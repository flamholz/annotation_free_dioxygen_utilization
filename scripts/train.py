...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature-type', '-f', type=str, default=None, choices=FEATURE_SUBTYPES + FEATURE_TYPES, help='The feature type on which to train.')
    ...
    result_path = os.path.join(RESULTS_PATH, f'train_{model_class}_{args.feature_type}_{task}.json')
    print(f'\nWriting results to {result_path}.')
    save_results_dict(results, result_path)

    model_path = os.path.join(MODELS_PATH, f'{model_class}_{args.feature_type}_{task}.joblib')
